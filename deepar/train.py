...
import torch
import argparse
from deepar.model import DeepAR, gaussian_likelihood_loss
...
def cli_flag_argparser():
    ...
    flag_parser.add_argument('--embedding_size', type=int, default=1)
...
def _forward(data, model, loss_fn, window, forecast_length, teacher_ratio):
    outputs = []
    label_x, feature_x, label_y, feature_y, _, ts_type_id = data
    ...
    for time_step in range(window):
        mu, sigma = model(inp[:, time_step:time_step + 1, :], ts_type_id)
        loss += loss_fn(mu, sigma, label_x[:, time_step])
    ...
    for idx in range(forecast_length):
        ...
        mu, sigma = model(inp, ts_type_id)
        output = torch.distributions.normal.Normal(mu, sigma).sample()
        outputs.append(output)
        loss += loss_fn(mu, sigma, y_true)
        # error = torch.mean(torch.abs(y_true - output) / torch.abs(y_true))
        # forecast_errors.append(error.detach().numpy())
    avg_loss = loss / (window + forecast_length)
...
def evaluate(val_loader, model, loss_fn, window, forecast_length):
    ...
    with torch.no_grad():
        for step, data in enumerate(val_loader):
            loss, outputs = _forward(data, model, loss_fn, window, forecast_length, 1.)
            losses.append(loss)
...
def train(raw, flags):
    ...
    model = DeepAR(cov_dim=data_processor.num_features, hidden_dim=flags.num_units,
                   num_layers=flags.num_layers, num_class=len(raw['type'].unique()),
                   embedding_dim=flags.embedding_size, batch_first=True, dropout=flags.dropout)
